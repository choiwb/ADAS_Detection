...
def ssd_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):
        ...
        if params['data_format'] == 'channels_first':
            # cls_pred = [tf.transpose(pred, [0, 2, 3, 1]) for pred in cls_pred] # CPU 변환
            # location_pred = [tf.transpose(pred, [0, 2, 3, 1]) for pred in location_pred] # CPU 변환

            cls_pred = [tf.transpose(pred, [0, 3, 1, 2]) for pred in cls_pred] # GPU 변환
            location_pred = [tf.transpose(pred, [0, 3, 1, 2]) for pred in location_pred] # GPU 변환
...
def main(_):
    ...
    replicate_ssd_model_fn = tf_replicate_model_fn.replicate_model_fn(ssd_model_fn, loss_reduction=tf.losses.Reduction.MEAN)
